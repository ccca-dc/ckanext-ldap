...
###################################################################################
# Anja 24.9.2018
def add_ckan_user_auto_register(context,data_dict):

    print "***************+++++++++++++++++++ add_ckan_user_auto_register"
    print data_dict
    ...
    #     cnx = ldap.initialize(config['ckanext.ldap.uri'])
    # else:
    #     raise ValidationError({ 'ckan.ldap.uri': 'not specified in development/production.ini'})
    #
    # check_name = data_dict['name']
    #
    # if config.get('ckanext.ldap.auth.dn'):
    ...
    # Anja: Waiting for email adress verification - no login possible
    ...
    ckan_user = plugins.toolkit.get_action('user_create')(context=context, data_dict=data_dict)
    print "Hello2"
    print ckan_user
    # Add the user to it's group if needed - JUST ONE LDAP SPECIFC ORGANIZATION - not for us; Anja 21.6.17
    if 'ckanext.ldap.organization.id' in config:
    ...
    user_org = _check_mail_org(data_dict['email'])
    ...
    # Anja 24.9.2018 : FIXME - Wofür war das????
    # #' TODO': check oben if already an entry for ldap_id!!!!!
    # # Add LdapUser entry = Database entry with match ckan_id - ldap_id
    # ldap_user = LdapUser(user_id=ckan_user['id'], ldap_id = ldap_user_dict['username'])
    # ckan.model.Session.add(ldap_user)
...
def ldap_mail_org(context, data_dict):
    ccca_orgs= [u'aau', u'ages', u'ait', u'alps',
                ...
                u'tu-graz', u'tu-wien',  u'uibk', u'uma', u'uni-salzburg',
